...
export default function ShopPage({ category, setCategory, search, setSearch, cart, cartLoading, cartSyncing, cartError, changeQuantity, count, total, navigate }) {
  ...
  return <section className="shop-page wrap" aria-labelledby="shop-title">
    ...
    {count > 0 && <div className="shop-mobile-cart"><div><span>{cartLoading ? 'Restoring cart…' : cartError ? 'Cart needs attention' : cartSyncing ? 'Saving cart…' : `${count} ${count === 1 ? 'item' : 'items'} in cart`}</span><strong>{money(total)}</strong></div><button className="primary" disabled={cartLoading} onClick={() => navigate('checkout')}>View cart <ShopIcon name="arrow" /></button></div>}
  </section>
}
