...
export default function HomePage({ customer, user, history, navigate, reorder, cart, quantity, changeQuantity, cartLoading }) {
  return <div className="home-page">
    ...
    <section className="home-hero-shell wrap">
      <div className="home-hero">
        <div className="home-hero-copy">
          ...
          <div className="home-hero-actions"><button className="primary" onClick={() => navigate('shop')}>Shop water <span aria-hidden="true">↗</span></button><a className="home-how-link" href="#home-how-it-works">How it works <span aria-hidden="true">↓</span></a></div>
    ...
    <section className="home-favourites wrap" aria-labelledby="home-favourites-heading">
      <div className="home-section-heading"><div><span className="eyebrow">FOR EVERY KIND OF THIRST</span><h2 id="home-favourites-heading">Your everyday essentials.</h2><p>Stock the fridge. Fill the dispenser. Get on with your day.</p></div><button className="home-all-products" onClick={() => navigate('shop')}>Shop all water <span aria-hidden="true">↗</span></button></div>
      <div className="home-product-grid">{products.slice(0, 4).map(product => <article className="home-product-card" key={product.id}>
        ...
        <div className="home-product-copy"><h3>{product.name}</h3><p>{product.detail}</p><div className="home-product-bottom"><strong>{money(product.price)}</strong>{cart[product.id] > 0 ? quantity(product) : <button className="home-add-button" disabled={cartLoading} aria-label={`Add ${product.name} to cart`} onClick={() => changeQuantity(product.id, 1)}>Add <span aria-hidden="true">+</span></button>}</div></div>
      </article>)}</div>
