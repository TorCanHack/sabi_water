...
import { emptyAddress, normalizeAddress, addressText, isCovered, validateAddress, validateSchedule, deliveryText } from '../../shared/commerce.mjs'
import { WalletReturn } from './Wallet'
...
function MenuIcon({ page }) {
  if (page === 'shop') return <CartIcon />
  if (page === 'orders') return <ReceiptIcon />
  const paths = {
    landing: <><path d="m3 10 9-7 9 7" /><path d="M5 9v12h5v-7h4v7h5V9" /></>,
    subscriptions: <><path d="M20 7h-6m6 0V1M4 17h6m-6 0v6" /><path d="M4 8a8 8 0 0 1 14-4l2 3M4 17l2 3a8 8 0 0 0 14-4" /></>,
    account: <><circle cx="12" cy="8" r="4" /><path d="M4 21v-2a8 8 0 0 1 16 0v2" /></>,
  }
  return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" focusable="false">{paths[page]}</svg>
}
  ...
  async function submitOrder(event) {
    ...
    try {
      ...
      if (payment !== 'delivery') {
        ...
        if (!currentConfig.enabled && payment !== 'wallet') throw new Error('Online payments are not configured yet.')
        ...
        if (pending.paymentStatus === 'paid') {
          ...
          if (pending.paymentSource === 'wallet' && pending.status !== 'Cancelled') { setOrder(pending); navigate('confirmation') }
