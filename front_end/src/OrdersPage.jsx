...
function OrdersIcon({ name }) {
  if (name === 'receipt') return <ReceiptIcon />
  const paths = {
...
function ProductImage({ line }) {
  ...
  return product?.image ? <img src={product.image} alt={line.name} width="64" height="64" loading="lazy" /> : <OrdersIcon name="receipt" />
}
...
function OrderCard({ order, reorder, cartLoading }) {
  ...
  return <article className="orders-card" aria-label={`Order ${order.reference}`}>
    ...
    <div className="orders-card-actions"><button className="orders-receipt" onClick={() => downloadReceipt(order)}><OrdersIcon name="receipt" />{order.paymentStatus === 'paid' ? 'Download receipt' : 'Download order summary'}</button><button className="orders-reorder" disabled={cartLoading} onClick={() => reorder(order)}><CartIcon />Order again</button></div>
  </article>
}
...
export default function OrdersPage({ history, navigate, reorder, customer, user, cartLoading }) {
  ...
  return <section className="orders-page wrap" aria-labelledby="orders-heading">
    <div className="orders-page-heading"><div><span className="eyebrow">EVERY WATER RUN, IN ONE PLACE</span><h1 id="orders-heading">My orders</h1><p>Follow your deliveries, find your favourites, and make your next refill easy.</p></div><button className="primary" onClick={() => navigate('shop')}>Shop water <CartIcon /></button></div>
    {!user && <div className="orders-guest-notice"><span className="orders-icon-tile"><OrdersIcon name="receipt" /></span><div><strong>Your orders, wherever you are.</strong><p>These are test orders saved in this browser. Sign in to view orders saved to your account.</p></div><button onClick={() => navigate('signin')}>Sign in <span aria-hidden="true">↗</span></button></div>}
    <div className="orders-overview" aria-label="Order overview">{[['receipt', 'All orders', 'Your water runs'], ['delivery', 'In progress', 'From checkout to your door'], ['check', 'Delivered', 'Completed deliveries']].map(([icon, label, detail]) => <div key={label}><span className="orders-icon-tile"><OrdersIcon name={icon} /></span><div><span>{label}</span><strong>{customer.loading ? '—' : counts[label]}</strong><small>{detail}</small></div></div>)}</div>
    ...
    {customer.loading ? <div className="orders-loading" aria-hidden="true">{[0, 1].map(item => <div key={item}><span /><span /><span /></div>)}</div> : visibleOrders.length ? <div className="orders-list">{visibleOrders.map(order => <OrderCard key={order.reference} order={order} reorder={reorder} cartLoading={cartLoading} />)}</div> : <div className="orders-empty"><span className="orders-icon-tile"><OrdersIcon name={history.length ? 'search' : 'receipt'} /></span><h3>{history.length ? 'No orders to show here.' : customer.error ? 'Your orders are unavailable.' : 'Your first water run starts here.'}</h3><p>{history.length ? 'Try another order status or search by product name or order reference.' : customer.error ? 'Try refreshing your orders again in a moment.' : 'Once you place an order, you can follow its progress and reorder your favourites here.'}</p>{history.length ? <button className="primary" onClick={clearFilters}>Show all orders <ReceiptIcon /></button> : !customer.error && <button className="primary" onClick={() => navigate('shop')}>Find your water <CartIcon /></button>}</div>}
